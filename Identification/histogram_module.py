...
def normalized_hist(img_gray, num_bins):
    ...
    for i in x:
        index = np.ceil(i / bin_size).astype(int)-1
        hists[index] += 1
    bins = np.linspace(0, 255, 41)
...
def rgb_hist(img_color_double, num_bins):
    ...
    bin_size = 255 / num_bins
    img_reshape = img_color_double.reshape(img_color_double.shape[0] * img_color_double.shape[1], img_color_double.shape[2])
    ...
    for i in img_reshape:
        ...
        hists[indices[0], indices[1], indices[2]] += 1
